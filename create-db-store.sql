...
-- $ = end
-- | = or
-- [abcd] before or after searching the string
-- [a-g] range before or after searching the string 
...
-- last name starts with my or se
select *
from customers
...
select o.order_id,c.first_name, sum(oi.quantity * oi.unit_price) as total_price
from customers c
...
select product_id,
name,
count(order_id) as abc,
if(count(order_id) > 1, 'Many','Once')
from products p
